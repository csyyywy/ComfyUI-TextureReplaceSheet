...
- 内置 CSV：插件目录里已附带 `texture-replace-sheet.csv`，开箱即用；
  你可以直接替换插件目录里的 CSV 文件（缓存自动失效），也可以把 `csv_path`
  指向任意其他路径（含相对路径，会优先相对插件目录解析）。
...
节点 `index` 填 1 时输出 CSV 第一行（贴图名称、对应内容、宽、高、提示词），
具体内容以你插件目录里的 `texture-replace-sheet.csv` 为准。
